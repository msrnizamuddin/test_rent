...
// A vehicle can have more than one fuel type (e.g. hybrid + electric).
const FUEL_TYPES = ["petrol", "diesel", "cng", "electric", "hybrid"];
const fuelTypeSchema = Joi.array()
  .items(Joi.string().valid(...FUEL_TYPES))
  .min(1);

// ---------------- 2.1 Vehicle Search + 2.2 Vehicle Filter (combined) ----------------
// GET /vehicles?search=&brand=&category=&location=&vehicleType=&seatingCapacity=
//               &minPrice=&maxPrice=&isAC=&transmission=&fuelType=&availability=
//               &distanceKm=&tripType=&page=&limit=&sortBy=&sortOrder=
export const searchVehicleValidation = Joi.object({
  // 2.1 Search
  search: Joi.string().trim().optional(), // matches vehicleName / brand (text search)
  brand: Joi.string().trim().optional(),
  categoryId: objectId.optional(),
  location: Joi.string().trim().optional(), // matches city / district / address
  // The "to" location — not used to filter vehicles (only `location`, the
  // pickup point, is), just accepted so Joi doesn't reject the request.
  ...
  fuelType: Joi.alternatives()
    .try(
      Joi.string().valid(...FUEL_TYPES),
      Joi.array().items(Joi.string().valid(...FUEL_TYPES)),
    )
    .optional(),
  availability: Joi.string()
    .valid("available", "assigned", "on-trip", "maintenance")
  ...
  // Pricing inputs — these never filter which vehicles come back, they're
  // only used (in vehicle.service.js) to compute each matched vehicle's
  // per-vehicle price: distanceKm feeds the per-km cost, tripType picks a
  // trip-type-specific PricingRule when one exists (e.g. a discount-trip
  // rate) instead of the generic one.
  distanceKm: Joi.number().min(0).optional(),
  tripType: Joi.string().valid("round", "oneway", "discount").optional(),
  // Not used for filtering or pricing today — accepted so Joi doesn't
  // reject the whole request just because the frontend also sends the
  // trip's pickup date/time alongside the pricing inputs above.
  pickupDate: Joi.string().trim().optional(),
  pickupTime: Joi.string().trim().optional(),
...
export const createVehicleValidation = Joi.object({
  vehicleName: Joi.string()
    .trim()
    .required()
    .messages(requiredMessage("Vehicle name")),
  brand: Joi.string().trim().required().messages(requiredMessage("Brand")),
  vehicleModel: Joi.string()
    .trim()
    .required()
    .messages(requiredMessage("Vehicle model")),
  categoryId: objectId.required().messages(requiredMessage("Category")),
  vehicleType: Joi.string()
    .valid(
      "sedan",
  ...
  registrationNumber: Joi.string()
    .trim()
    .required()
    .messages(requiredMessage("Registration number")),
  modelYear: Joi.number()
    .integer()
    .min(1980)
    .max(new Date().getFullYear() + 1)
  ...
  seatingCapacity: Joi.number()
    ...
    .min(1)
    .required()
    .messages(requiredMessage("Seating capacity")),
  fuelType: fuelTypeSchema.required().messages(requiredMessage("Fuel type")),
  transmission: Joi.string()
    .valid("manual", "automatic")
    .required()
    .messages(requiredMessage("Transmission")),
  isAC: Joi.boolean().optional(),
  // New vehicles command a higher price than old ones in the same
  // category — used to pick the right PricingRule. Defaults to "new".
  condition: Joi.string().valid("new", "old").optional(),
    ...
    .items(
      Joi.object({
        title: Joi.string()
          .trim()
          .required()
          .messages(requiredMessage("Document title")),
        fileUrl: Joi.string()
          .uri()
          .required()
          .messages(requiredMessage("Document file URL")),
      }),
    )
    .optional(),
  // superadmin/manager can create directly as available; otherwise defaults to pending review
